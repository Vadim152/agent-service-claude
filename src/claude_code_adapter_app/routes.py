...
async def create_run(payload: AdapterRunCreateRequest, request: Request) -> AdapterRunCreateResponse:
    return await asyncio.to_thread(_service(request).create_run, payload)
...
async def get_run(backend_run_id: str, request: Request) -> AdapterRunStatusResponse:
    return await asyncio.to_thread(_service(request).get_run, backend_run_id)
...
async def list_events(
    ...
    return await asyncio.to_thread(_service(request).list_events, backend_run_id, after=after, limit=limit)
...
async def cancel_run(backend_run_id: str, request: Request) -> AdapterRunCancelResponse:
    return await asyncio.to_thread(_service(request).cancel_run, backend_run_id)
...
async def submit_approval_decision(
    ...
    return await asyncio.to_thread(_service(request).submit_approval_decision, backend_run_id, approval_id, payload)
...
async def ensure_session(payload: AdapterSessionEnsureRequest, request: Request) -> AdapterSessionDto:
    return await asyncio.to_thread(_service(request).ensure_session, payload)
...
async def get_session(external_session_id: str, request: Request) -> AdapterSessionDto:
    return await asyncio.to_thread(_service(request).get_session, external_session_id)
...
async def compact_session(external_session_id: str, request: Request) -> AdapterSessionCommandResponse:
    return await asyncio.to_thread(_service(request).compact_session, external_session_id)
...
async def get_session_diff(external_session_id: str, request: Request) -> AdapterSessionDiffResponse:
    return await asyncio.to_thread(_service(request).get_session_diff, external_session_id)
...
async def execute_session_command(
    ...
    return await asyncio.to_thread(_service(request).execute_session_command, external_session_id, payload)
